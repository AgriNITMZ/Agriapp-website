...
const Cart = () => {
  ...
  const fetchProduct = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/products/cartitems`, {
 ...
 const handleAddressSelect = (address) => {
  ...
  const removeItem = async (id) => {
    try {
      ...
      await axios.delete(`${import.meta.env.VITE_API_URL}/products/removeitem/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
        ...
        <div className="flex flex-col lg:flex-row gap-6 min-h-[calc(100vh-8rem)]">
          ...
          <div className="lg:w-2/5 w-full">
            <div className="bg-white rounded-lg shadow-sm p-6 sticky top-4">
              ...
              <button
                className="w-full mt-3 bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 transition-all duration-300 font-medium flex items-center justify-center gap-2 border-2 border-blue-700"
                onClick={() => {
                  if (!cartProduct?.items || cartProduct.items.length === 0) {
                    toast.error('Your cart is empty');
                    return;
                  }
                  // Convert cart items to Shiprocket format
                  const shiprocketItems = cartProduct.items.map(item => ({
                    productId: item.product._id,
                    name: item.product.name,
                    quantity: item.quantity,
                    price: item.selectedDiscountedPrice,
                    imageUrl: item.product.images?.[0] || ''
                  }));
                  navigate('/shiprocket/checkout', {
                    state: {
                      cartItems: shiprocketItems
                    }
                  });
                }}
              >
                <span>Checkout with Shiprocket</span>
                <ArrowRight className="w-5 h-5" />
              </button>
            </div>
          </div>
